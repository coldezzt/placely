...
DONE?:  проблема с удалением списков цен - они просто генерируются, а при удалении имущества список остаётся
        по идее после настройки адекватных свзяей они должны начать удаляться
...
DONE:   добавить примеры путей для контроллера например: [HttpGet("my")] // api/chat/my/ - т.е. почти полный путь рядом с контроллером
        добавлены

DONE:   все `tenant` заменить на `user`
        заменено, миграции добавлены
        
DONE:   убрать `public` в интерфейсах
        убрано

*/

/*
 TODO: [IN PROGRESS] заменить перебор значений из enum на приватный метод применения фильтров в методе фильтрации имуществ
