    ...
    : Repository<Review>(logger, appDbContext), IReviewRepository
{
    public async Task<Review?> TryFindByAuthorIdAndPropertyId(long authorId, long propertyId)
    {
        ...
        logger.Log(LogLevel.Debug, "By this author and property: {authorId} and {propertyId}, " +
    ...
    {
        logger.Log(LogLevel.Trace, $"Begin getting reviews list of property with Id: {propertyId}");
